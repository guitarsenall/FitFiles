...
# fitfilepath = FileLoc + r'2018-04-16-17-52-22.fit' # Z3 intervals
# fitfilepath = FileLoc + r'2018-04-14-16-03-28.fit'  # road race
# fitfilepath = FileLoc + r'2018-04-19-14-28-03.fit'  # Z4 intervals
# fitfilepath = FileLoc + r'2018-04-20-11-31-14.fit'  # spin-ups, sprint
fitfilepath = FileLoc + sys.argv[1]
...
ax.plot_date( x, fboxpower, 'r.');
ax.plot_date( x, zone_mid, 'g-');
ax.xaxis.set_major_formatter(DateFormatter('%H:%M:%S'))
ax.set_yticks( p_zone_bounds, minor=False)
fig.autofmt_xdate()
plt.legend(['power', 'filtered power', 'zone mid'], loc='upper left');
plt.grid()
plt.show()
'''
...
'''
# This histogram method is flexible for graphics, but it does not allow
# access to the counts for manipulation.
# histogram plot. See
# https://matplotlib.org/gallery/statistics/histogram_multihist.html
data = array([ power, zone_mid ]).transpose()
fig, ax = plt.subplots()
bins    = p_zone_bounds
#ax.hist(data, bins, density=True, histtype='step', fill=False,
#        stacked=True, rwidth=0.8)
ax.hist(data, bins, density=False, histtype='bar', color=['red', 'blue'],
        label=['raw power', 'detected zone'],
        rwidth=0.8)
